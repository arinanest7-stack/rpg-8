      ...
      <div className="rune-frame relative flex w-full max-w-4xl h-[90vh] flex-col rounded-2xl bg-card border border-primary/50 shadow-2xl overflow-hidden">
        ...
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-border/60 bg-background/80 px-6 py-4">
          <div>
            ...
            <h2 className="font-display text-xl uppercase tracking-wider text-foreground mt-1">
              {currentStep.title}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-3 rounded-xl border border-gold/40 bg-gold/10 px-3.5 py-1.5 font-mono text-xs text-gold">
              <span className="flex items-center gap-1">
                <Zap className="h-3.5 w-3.5" /> +{currentStep.xpReward || 25} XP
              </span>
              <span>•</span>
              <span className="flex items-center gap-1">
                <Coins className="h-3.5 w-3.5" /> +{currentStep.goldReward || 5}g
              </span>
            </div>
        ...
        {showSuccessBanner && (
          <div className="bg-[linear-gradient(90deg,var(--xp),color-mix(in_oklab,var(--xp)_40%,transparent))] px-6 py-2.5 text-center font-mono text-xs text-background font-semibold animate-pulse flex items-center justify-center gap-2">
            ...
            <span>Step Completed! +{currentStep.xpReward || 25} XP & +{currentStep.goldReward || 5} Gold earned!</span>
          </div>
        )}
        ...
        <main className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
          <p className="text-xs text-muted-foreground font-mono leading-relaxed border-b border-border/40 pb-3">
            {currentStep.desc || "Review the study blocks below, complete checklists, and click Complete Step."}
          </p>
                  ...
                  <div className="flex flex-col gap-3">
                    {block.elements.map((el) => {
                      ...
                      if (el.type === "bullet_list")
                        return (
                          <li key={el.id} className="ml-4 list-disc text-xs text-foreground/90 leading-relaxed">
                            {el.content}
                          </li>
                        );
                      if (el.type === "numbered_list")
                        return (
                          <li key={el.id} className="ml-4 list-decimal text-xs text-foreground/90 leading-relaxed">
                            {el.content}
                          </li>
                        );
                      if (el.type === "todolist")
                      ...
                      return (
                        <p key={el.id} className="text-xs text-foreground/90 leading-relaxed">
                          {el.content}
                        </p>
                      );
                    })}
